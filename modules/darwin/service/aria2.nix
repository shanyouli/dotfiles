...
in {
  ...
  config = mkIf cfg.enable {
    modules.download.aria2.enable = true;
    modules.download.aria2.aria2p = mkDefault true;
    launchd.user.agents.aria2 = {
      path = ["${config.modules.download.aria2.package}/bin" config.modules.service.path];
      serviceConfig.ProgramArguments = [
        "${config.modules.download.aria2.package}/bin/aria2c"
