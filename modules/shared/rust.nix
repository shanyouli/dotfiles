...
in {
  options.my.modules.rust = with types; {
  ...
  config = mkIf cfg.enable (mkMerge [
    (mkIf cfg.rustup.enable {
      ...
      my.modules.zsh.env.RUSTUP_HOME = rustup_dir;
      #TODO: rustup install stable and rust-analyzer
      my.modules.rust.rustup.script = ''
      ...
      my.user.packages = with pkgs;
        [
          (rust-bin.stable.latest.default.override {
            extensions = [ ] ++ optionals config.my.modules.dev.enable [
              "rust-src"
              "rust-analyzer"
            ];
          })
