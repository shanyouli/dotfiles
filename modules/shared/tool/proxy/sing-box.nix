...
with lib.my; let
  # TODO: 设置更好的管理配置文件的方法
  cfg = config.modules.tool.proxy.sing-box;
  workdir = "${config.home.cacheDir}/sing-box";
in {
  options.modules.tool.proxy.sing-box = {
  ...
  config = mkIf cfg.enable {
    ...
    modules.tool.proxy.sing-box.serviceCmd = ''sudo ${cfg.package}/bin/sing-box run -c "${cfg.configFile}" -D "${workdir}"'';
  };
}
