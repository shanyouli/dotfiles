...
in {
  ...
  config = mkIf cfg.enable {
    ...
    modules.shell.aliases.log = "lnav";
    modules.shell.zsh = {
      envInit = mkOrder 100 ''
        # This file is autogenerated, do not edit it!
        ${concatStringsSep "\n" (mapAttrsToList (n: v: (
            if (strings.toUpper "${n}") == "PATH"
            then ''export path=(${concatMapStringsSep " " builtins.toString v} $path)''
            else
              (
                if builtins.isList v
                then ''export ${n}=${concatMapStringsSep ":" builtins.toString v}''${n:+:}''${n}''
                else ''export ${n}="${v}"''
              )
          ))
          cfp.env)}
    ...
    home.configFile =
      {
        "zsh".source = "${lib.var.dotfiles.config}/zsh";
        "zsh".recursive = true;
