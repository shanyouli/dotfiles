...
in
{
  options.modules.app.editor.helix = {
    ...
    languages =
      with types;
      mkOpt' (coercedTo (listOf tomlFormat.type) (
        language:
        lib.warn ''
        ...
        '' { inherit language; }
      ) (addCheck tomlFormat.type builtins.isAttrs)) { } "lsp support";
    ignores = with types; mkOpt' (listOf str) [ ] "ignores .gitignore files.";
    themes =
      mkOpt' (types.attrsOf tomlFormat.type) { }
        "Each theme is written to ~/.config/helix/themes/xx.toml";
  };
