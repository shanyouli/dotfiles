...
with my; let
  ...
  pluginsOptionType = let
    inherit (types) listOf oneOf package str submodule;
  in
    listOf (oneOf [
      package
      (submodule {
  ...
  treesit-list = let
    ...
    inherit (builtins) isString isList filter elem attrNames;
  in
    if isString cfg.treesit
    then
      (
        if cfg.treesit == "all"
        then
          ...
          (
            let
              ignore-list = ["org"];
              all-treesit = attrNames (lib.filterAttrs (name: v: (
                  !(elem name ignore-list) && lib.isDerivation v
                ))
                nvim-treesitter-parsers);
            in
              nvim-treesitter.withPlugins (plugins: attrVals all-treesit plugins)
          )
        else if nvim-treesitter-parsers ? cfg.treesit
        then nvim-treesitter.withPlugins (plugins: attrVals [cfg.treesit] plugins)
        else null
      )
    else if isList cfg.treesit
    then
      (let
...
in {
  options = with lib; {
    modules.app.editor.nvim = {
      ...
      treesit = mkOption {
        default = null;
        type = with types; oneOf [(nullOr str) (listOf (oneOf [str package]))];
      };
