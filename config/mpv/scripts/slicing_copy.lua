...
local function cut(shift, endpos)
	...
	local outpath = utils.join_path(o.target_dir, get_outname(shift, endpos))
	local ua = mp.get_property("user-agent")
	local referer = mp.get_property("referrer")
	local cmds = Command:new(o.ffmpeg_path):arg("-v", "warning"):arg(o.overwrite and "-y" or "-n"):arg("-stats")
