...
local function generate_m3u(magnet_uri, files)
	for _, fileinfo in ipairs(files) do
		...
		fileinfo.path = {}
		for w in fileinfo.fullpath:gmatch("([^/]+)") do
			table.insert(fileinfo.path, w)
		end
	...
	for _, fileinfo in ipairs(files) do
		if fileinfo.processed ~= true then
			...
			local url = opts.server
				.. "/stream/"
				.. urlencode(fileinfo.fullpath)
				.. "?play&index="
				.. fileinfo.id
				.. "&link="
				.. infohash
			...
			if opts.search_for_external_tracks and basename ~= nil and fileinfo.type == "video" then
				...
				for _, fileinfo2 in ipairs(files) do
					if
						#fileinfo2.path > 0
						and fileinfo2.type ~= "other"
						and fileinfo2.processed ~= true
						and string.find(fileinfo2.path[#fileinfo2.path], basename, 1, true) ~= nil
					then
						mp.msg.info("->" .. fileinfo2.fullpath)
						local title = string_replace(fileinfo2.fullpath, basename, "%")
						local url = opts.server
							.. "/stream/"
							.. urlencode(fileinfo2.fullpath)
							.. "?play&index="
							.. fileinfo2.id
							.. "&link="
							.. infohash
