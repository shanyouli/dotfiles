...
local function build_track_title(track, prefix, filename)
    ...
    if track.external and title ~= "" then
        ...
        if title:lower() == codec:lower() then
            title = ""
        end
    end
    ...
    if track["demux-fps"] then
        h(string.format("%.5g fps", track["demux-fps"]))
    end
    if track["audio-channels"] then
        h(track["audio-channels"] .. " ch")
    end
    if track["demux-samplerate"] then
        h(string.format("%.3g kHz", track["demux-samplerate"] / 1000))
    end
    if track["demux-bitrate"] then
        h(string.format("%.0f kbps", track["demux-bitrate"] / 1000))
    end
    if #hints > 0 then
        title = string.format("%s [%s]", title, table.concat(hints, ", "))
    end
    ...
    if prefix then
        title = string.format("%s: %s", type:sub(1, 1):upper(), title)
    end
    return title
end
    ...
    for _, track in ipairs(list) do
        if track.type == type then
            ...
            if track.selected and track.id == pos then
                ...
                if type == "sub" then
                    if
                        (prop == "sid" and not get("sub-visibility"))
                        or (prop == "secondary-sid" and not get("secondary-sub-visibility"))
                    then
...
local function update_tracks_menu(menu)
    ...
    for _, item in ipairs(items_v) do
        append_menu(submenu, item)
    end
    if #submenu > 0 and #items_a > 0 then
        append_menu(submenu, { type = "separator" })
    end
    for _, item in ipairs(items_a) do
        append_menu(submenu, item)
    end
    if #submenu > 0 and #items_s > 0 then
        append_menu(submenu, { type = "separator" })
    end
    for _, item in ipairs(items_s) do
        append_menu(submenu, item)
    end
end
...
local function update_chapters_menu(menu)
    ...
    for id, chapter in ipairs(chapter_list) do
        ...
        if title == "" then
            title = "Chapter " .. string.format("%02.f", id)
        end
...
local function update_editions_menu(menu)
    ...
    for id, edition in ipairs(edition_list) do
        ...
        if title == "" then
            title = "Edition " .. string.format("%02.f", id)
        end
...
local function update_playlist_menu(menu)
    ...
    if o.max_playlist_items > 0 then
        ...
        if to > #playlist then
            from, to = #playlist - o.max_playlist_items + 1, #playlist
        end
    end
    ...
    for _, profile in ipairs(profile_list) do
        if
            not (
                ...
                or profile.name:find("gui")
                or profile.name == "encoding"
                or profile.name == "libmpv"
            )
        then
            append_menu(submenu, {
                title = profile.name,
...
local dyn_updaters = {
    ...
    ["tracks/video"] = function(menu)
        update_track_menu(menu, "video", "vid")
    end,
    ["tracks/audio"] = function(menu)
        update_track_menu(menu, "audio", "aid")
    end,
    ["tracks/sub"] = function(menu)
        update_track_menu(menu, "sub", "sid")
    end,
    ["tracks/sub-secondary"] = function(menu)
...
local function parse_input_conf(conf)
    local function parse_line(line)
        ...
        if c and not o.uosc_syntax then
            return
        end
    ...
    local function extract_title(cmd)
        ...
        if not title and o.uosc_syntax then
            title = cmd:match("#!%s*(.*)%s*")
        end
        if title then
    ...
    local function split_title(title)
        ...
        if last_ends < (#title + 1) then
            list[#list + 1] = title:sub(last_ends)
        end
...
mp.register_script_message("update", function(keyword, json)
    ...
    if err then
        msg.error("update: failed to parse json:", err)
    end
