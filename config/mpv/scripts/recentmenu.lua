...
function write_json()
    ...
    if dyn_menu.ready and (os_name == "windows") then
...
function append_item(path, filename, title)
    ...
    for index, value in ipairs(menu.items) do
        ...
        if #new_items < o.length and path ~= opath and not is_same_series(path, opath) then
...
function on_load()
    ...
    if path:match("bd://") or path:match("dvd://") or path:match("dvb://") or path:match("cdda://") then
        return
    end
    if not is_protocol(path) then
        path = normalize(path)
