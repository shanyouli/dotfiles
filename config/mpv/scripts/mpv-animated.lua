...
filters = string.format(
    "fps=%s,scale='trunc(ih*dar/2)*2:trunc(ih/2)*2',setsar=1/1,scale=%s:-1:flags=lanczos",
    fps,
    options.rez
)
...
if output_directory ~= "" then
    ...
    if not meta or not meta.is_dir then
        local windows_args =
            { "powershell", "-NoProfile", "-Command", "mkdir", string.format('"%s"', output_directory) }
        ...
        if res.status ~= 0 then
            msg.error(
                "Failed to create animated_dir save directory "
                    .. output_directory
                    .. ". Error: "
...
function make_animated_internal(burn_subtitles)
    ...
    if options.type == "webp" then
        arg = string.format(
            "%s -y -hide_banner -loglevel error -ss %s %s -t %s -i '%s' -lavfi %s -lossless %s -q:v %s -compression_level %s -loop %s '%s'",
            options.ffmpeg_path,
            position,
            copyts,
            duration,
        ...
        arg = string.format(
            "%s -y -hide_banner -loglevel error -ss %s %s -t %s -i '%s' -lavfi %s,'split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse' -loop %s '%s'",
            options.ffmpeg_path,
            position,
            copyts,
            duration,
