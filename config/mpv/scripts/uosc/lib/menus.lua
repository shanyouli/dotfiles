...
function open_command_menu(data, opts)
	...
	if opts then
		...
		if opts.on_close then
			menu_opts.on_close = function()
				run_command(opts.on_close)
			end
		end
	end
	local menu = Menu:open(data, run_command, menu_opts)
	if opts and opts.submenu then
		menu:activate_submenu(opts.submenu)
	end
	return menu
end
...
function create_select_tracklist_type_menu_opener(menu_title, track_type, track_prop, load_command, download_command)
	...
	local function serialize_tracklist(tracklist)
		...
		if track_type ~= "" then
			disabled_item =
				{ title = t("Disabled"), italic = true, muted = true, hint = "—", value = nil, active = true }
		...
		for _, track in ipairs(tracklist) do
			if track.type == track_type then
				...
				if track["codec"] then
					h(escape_codec(track.codec))
				end
				if track["audio-channels"] then
					h(
						track["audio-channels"] == 1 and t("%s channel", track["audio-channels"])
	...
	return create_self_updating_menu_opener({
		...
		on_paste = function(path)
			load_track(track_type, path)
		end,
	})
end
...
function open_file_navigation_menu(directory_path, handle_select, opts)
	...
	for index, item in ipairs(items) do
		if not item.value.is_to_parent and opts.active_path == item.value then
			item.active = true
			if not opts.selected_path then
				selected_index = index
			end
		end

		if opts.selected_path == item.value then
			selected_index = index
		end
	end
	...
	if process.status == 0 then
		for _, value in ipairs(split(process.stdout, "\n")) do
			...
			if drive then
				...
				if opts.selected_path == drive_path then
	...
	function get_all_user_bindings()
		...
		for line in input_conf_iterator do
			...
			if
				...
				and (not is_commented_out or is_menu_item)
			then
...
function get_keybinds_items()
	...
	local no_key_menu_binds = itable_filter(get_all_user_bindings(), function(b)
...
function open_open_file_menu()
	...
	menu = open_file_navigation_menu(directory, function(path, mods)
...
function create_track_loader_menu_opener(opts)
	...
	return function()
		...
		local function handle_select(path)
			load_track(opts.prop, path)
		end
...
function open_subtitle_downloader()
	...
	if state.path then
		if is_protocol(state.path) then
			if not is_protocol(state.title) then
				search_suggestion = state.title
			end
		else
	...
	local function ensure_response_data(success, result, error, check)
		...
		if success and result and result.status == 0 then
			...
			if not data or not check(data) then
				data = (data and data.error == true) and data
					or {
						error = true,
		...
		if data.error then
			...
			if message_verbose then
				msg.error(message_verbose)
			end
			menu:update_items({
				{
					title = message,
	...
	handle_search = function(query, page)
		...
		mp.command_native_async({
			...
			local subs = itable_filter(data.data, function(sub)
				return sub
					and sub.attributes
					and sub.attributes.release
					and type(sub.attributes.files) == "table"
					and #sub.attributes.files > 0
			end)
