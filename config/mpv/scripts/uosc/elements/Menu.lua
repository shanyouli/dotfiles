...
---@param opts? MenuOptions
---@return Menu
function Menu:new(data, callback, opts)
	return Class.new(self, data, callback, opts) --[[@as Menu]]
end
---@param data MenuData
---@param callback MenuCallback
---@param opts? MenuOptions
...
function Menu:update(data)
	...
	while i < #menus_to_serialize do
		...
		for i, item_data in ipairs(menu_data.items or {}) do
			if item_data.active and not first_active_index then
				first_active_index = i
			end

			local item = {}
			table_assign_props(item, item_data, item_props_to_copy)
			if item.keep_open == nil then
				item.keep_open = menu.keep_open
			end
	...
	for _, menu in ipairs(self.all) do
		if menu.search then
			...
			if not menu.on_search then
				self:search_internal(menu, true)
			end
		end
...
function Menu:update_content_dimensions()
	...
	for _, menu in ipairs(self.all) do
		...
		for _, item in ipairs(menu.items) do
			...
			local spacings_in_item = 1
				+ (item.title_width > 0 and 1 or 0)
				+ (item.hint_width > 0 and 1 or 0)
				+ (icon_width > 0 and 1 or 0)
			local estimated_width = item.title_width
				+ item.hint_width
				+ icon_width
				+ (self.item_padding * spacings_in_item)
			if estimated_width > max_width then
...
function Menu:scroll_to(pos, menu, fling_options)
	...
	if fling_options then
		table_assign(menu.fling, fling_options)
	end
	request_render()
end
...
function Menu:scroll_by(delta, menu, fling_options)
	...
	if menu ~= nil then
		self:scroll_to(
			(menu.fling and (menu.fling.y + menu.fling.distance) or menu.scroll_y) + delta,
			menu,
			fling_options
		)
	end
...
function Menu:select_value(value, menu)
	menu = menu or self.current
	local index = itable_find(menu.items, function(item)
...
function Menu:open_selected_item(opts)
	...
	if menu.selected_index then
		...
		if item.items then
			...
			if not item.keep_open and not opts.keep_open then
				self:close()
			end
		end
	end
...
function Menu:open_selected_item_soft()
	self:open_selected_item({ keep_open = true })
end
function Menu:open_selected_item_preselect()
	self:open_selected_item({ preselect_first_item = true })
end
...
function Menu:move_selected_item_up()
	if self.current.selected_index then
		self:move_selected_item_to(self.current.selected_index - 1)
	end
end
...
function Menu:delete_selected_item()
	...
	if callback and index then
		callback(index, self.current.submenu_path)
	end
end

function Menu:on_display()
	self:update_dimensions()
end
function Menu:on_prop_fullormaxed()
	self:update_content_dimensions()
end
function Menu:on_options()
	self:update_content_dimensions()
end
...
function Menu:select_by_offset(offset, menu)
	...
	local prev_index = itable_find(menu.items, function(item)
		return item.selectable ~= false
	end, index, 1)
	local next_index = itable_find(menu.items, function(item)
		return item.selectable ~= false
	end, index)
...
function Menu:navigate_by_offset(offset, immediate)
	...
	if self.current.selected_index then
		self:scroll_to_index(self.current.selected_index, self.current, immediate)
	end
end
	...
	for _, item in ipairs(items) do
		if item.selectable ~= false then
			...
			if item.items and recursive then
				...
				if
					title and title:find(query, 1, true)
					or title and romanization and ligature_conv_title:find(query, 1, true)
					or hint and hint:find(query, 1, true)
					or title and initials_title:find(query, 1, true)
					or title and romanization and initials_conv_title:find(query, 1, true)
					or hint and concat(initials(hint)):find(query, 1, true)
				then
...
function Menu:search_text_input(info)
	local menu = self.current
	if not menu.search and menu.search_style == "disabled" then
		return
	end
...
function Menu:search_init(menu)
	...
	if menu.search_debounce ~= "submit" and menu.search_debounce > 0 then
		timeout = mp.add_timeout(
			menu.search_debounce / 1000,
			self:create_action(function()
				self:search_submit(menu)
			end)
		)
		timeout:kill()
...
function Menu:search_start(menu)
	if (menu or self.current).search_style == "disabled" then
		return
	end
	self:search_init(menu)
	self:search_ensure_key_bindings()
...
function Menu:search_enable_key_bindings()
	if #self.key_bindings_search ~= 0 then
		return
	end
...
function Menu:search_disable_key_bindings()
	for _, name in ipairs(self.key_bindings_search) do
...
function Menu:enable_key_bindings()
	...
	self:add_key_binding(
		"bs",
		"menu-back-alt4",
		self:create_key_action("key_bs"),
		{ repeatable = true, complex = true }
	)
	self:add_key_binding(
		"shift+bs",
		"menu-clear-query",
		self:create_key_action("key_bs", { shift = true }),
		{ repeatable = true, complex = true }
	)
	self:add_key_binding(
		"ctrl+bs",
		"menu-delete-word",
		self:create_key_action("key_bs", { ctrl = true }),
		{ repeatable = true, complex = true }
	)
...
function Menu:is_alive()
	return not self.is_closing and not self.is_closed
end
		...
		for index = start_index, end_index, 1 do
			...
			if item_by < by and ((not has_background and not next_has_background) or item.separator) then
				...
				ass:rect(ax + spacing, separator_ay, bx - spacing, separator_by, {
					color = fg,
					opacity = menu_opacity * (item.separator and 0.13 or 0.04),
				})
			end
		...
		if draw_title then
			...
			if menu.search then
				ass:rect(ax + 3, rect.ay + 3, bx - 3, rect.ay + title_height - 1, {
					color = fg .. "\\1a&HFF",
					opacity = menu_opacity * 0.1,
					radius = state.radius > 0 and state.radius + self.padding or 0,
					border = 1,
					border_color = fg,
					border_opacity = menu_opacity * 0.8,
				})
			...
			if menu.search then
				...
				if is_current and requires_submit then
					cursor:zone("primary_down", icon_rect, function()
						self:search_submit()
					end)
				...
				if menu.search.query ~= "" then
					...
					local placeholder = (menu.search_style == "palette" and menu.ass_safe_title) and menu.ass_safe_title
