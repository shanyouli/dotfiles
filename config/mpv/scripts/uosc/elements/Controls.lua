...
function Controls:new()
	return Class.new(self) --[[@as Controls]]
end
...
function Controls:init_options()
	...
	for i, item in ipairs(items) do
		...
		config = shorthands[config_tooltip[1]] and split(shorthands[config_tooltip[1]], " *%? *")[1]
			or config_tooltip[1]
		...
		elseif kind == "command" then
			if #params ~= 2 then
				...
				if badge then
					self:register_badge_updater(badge, element)
				end
			end
		...
		elseif kind == "button" then
			if #params ~= 1 then
				mp.error(
					string.format(
						"managed button needs 1 parameter, %d received: %s",
						#params,
						table.concat(params, "/")
					)
				)
...
function Controls:reflow()
	...
	for _, control in ipairs(self.controls) do
		...
		if control.element then
			control.element.enabled = matches
		end
		if matches then
...
function Controls:register_badge_updater(badge, element)
	...
	if is_external_prop then
		element["on_external_prop_" .. prop] = function(_, value)
			handler(prop, value)
		end
	else
...
function Controls:get_visibility()
	return Elements:v("speed", "dragging") and 1
...
function Controls:update_dimensions()
	...
	local available_space = display.height
		- window_border * 2
	...
	for c, control in ipairs(self.layout) do
		control.hide = false
		if control.element then
			control.element.enabled = self.enabled
		end
	end
	...
	for c, control in ipairs(self.layout) do
		if not control.hide then
			...
			if sizing == "space" then
				if individual_space_width > 0 then
					width = individual_space_width
				end
			elseif sizing == "gap" then
				if width_for_gaps > 0 then
					width = width_for_gaps * (ratio / gaps)
				end
			...
			elseif sizing == "dynamic" then
				height = size * scale
				width = max_dynamics_width < width_for_dynamics and height * ratio
...
function Controls:on_display()
	self:update_dimensions()
end
function Controls:on_prop_border()
	self:update_dimensions()
end
function Controls:on_prop_title_bar()
	self:update_dimensions()
end
function Controls:on_prop_fullormaxed()
	self:update_dimensions()
end
function Controls:on_timeline_enabled()
	self:update_dimensions()
end
