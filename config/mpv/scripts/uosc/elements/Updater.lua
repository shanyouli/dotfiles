...
function Updater:new()
    return Class.new(self) --[[@as Updater]]
end
...
function Updater:render()
    ...
    local divider_half, divider_border_half, divider_y =
        divider_width / 2, round(1 * state.scale), display.height * 0.65
    ...
    if self.state ~= "pending" then
        ...
        cursor:zone("primary_click", button_rect, function()
            self:destroy()
        end)
    end
