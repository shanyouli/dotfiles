...
function VolumeSlider:new(props)
    return Class.new(self, props) --[[@as VolumeSlider]]
end
...
function VolumeSlider:render()
    ...
    ass:new_event()
    ass:append(
        "{\\rDefault\\an7\\blur0\\bord0\\1c&H" .. bg .. "\\iclip(" .. fg_path.scale .. ", " .. fg_path.text .. ")}"
    )
...
function Volume:new()
    return Class.new(self) --[[@as Volume]]
end
...
function Volume:update_dimensions()
    ...
    local max_y = Elements:v("controls", "ay")
        or Elements:v("timeline", "ay")
...
function Volume:on_display()
    self:update_dimensions()
end
function Volume:on_prop_border()
    self:update_dimensions()
end
function Volume:on_prop_title_bar()
    self:update_dimensions()
end
function Volume:on_controls_reflow()
    self:update_dimensions()
end
function Volume:on_options()
    self:update_dimensions()
end
...
function Volume:render()
    ...
    cursor:zone("primary_click", mute_rect, function()
    ...
    ass:icon(
        mute_rect.ax + width_half,
        mute_rect.ay + height_half,
        icon_size,
        "volume_up",
        { border = options.text_border * state.scale, opacity = underlay_opacity, align = 5 }
    )
    ass:icon(
        mute_rect.ax + width_half - horizontal_shift,
        mute_rect.ay + height_half,
        icon_size,
        icon_name,
