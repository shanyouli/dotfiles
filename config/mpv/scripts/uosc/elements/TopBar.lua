...
function TopBar:new()
    return Class.new(self) --[[@as TopBar]]
end
...
function TopBar:decide_titles()
    ...
    if self.main_title and self.alt_title and not self.show_alt_title then
        ...
        if
            string.match(longer_title --[[@as string]], escaped_shorter_title)
        then
...
function TopBar:on_prop_title()
    self:decide_titles()
end
function TopBar:on_prop_alt_title()
    self:decide_titles()
end
...
function TopBar:render()
    ...
    if options.top_bar_controls then
        ...
        for _, button in ipairs(self.buttons) do
            if button.is_max then
                button.icon = state.fullscreen and "close_fullscreen"
                    or (state.maximized and "filter_none" or "crop_square")
            end
    ...
    if state.title or state.has_playlist then
        ...
        if state.has_playlist then
            ...
            local opacity = get_point_to_rectangle_proximity(cursor, rect) == 0 and 1
                or config.opacity.playlist_position
            if opacity > 0 then
        ...
        if title_bx - title_ax > self.font_size * 3 and options.top_bar_title ~= "no" then
            ...
            if main_title then
                ...
                if options.top_bar_alt_title_place == "toggle" then
                    cursor:zone("primary_click", title_rect, function()
                        self:toggle_title()
                    end)
                end
