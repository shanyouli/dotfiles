...
function CycleButton:new(id, props)
    return Class.new(self, id, props) --[[@as CycleButton]]
end
---@param id string
---@param props CycleButtonProps
function CycleButton:init(id, props)
    ...
    self.on_click = function()
        ...
        elseif is_state_prop then
            if itable_index_of({ "yes", "no" }, new_value) then
    ...
    local function handle_change(name, value)
        ...
        local index = itable_find(self.states, function(state)
