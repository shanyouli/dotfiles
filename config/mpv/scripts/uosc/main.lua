...
function update_config()
    ...
    for _, name in ipairs({ "timeline", "controls", "volume", "top_bar", "speed" }) do
        ...
        if type(value) == "string" then
            for _, state in ipairs(comma_split(value)) do
                flags[state] = true
            end
        end
    ...
    -- Opacity
    config.opacity = table_assign(
        {},
        config_defaults.opacity,
        serialize_key_value_list(options.opacity, function(value, key)
            return clamp(0, tonumber(value) or config.opacity[key], 1)
        end)
    )

    -- Color
    config.color = table_assign(
        {},
        config_defaults.color,
        serialize_key_value_list(options.color, function(value)
            ...
            items = {
                {
                    ...
                    value = "script-binding uosc/next",
                },
                {
                    title = t("Prev"),
                    hint = t("playlist or file"),
                    value = "script-binding uosc/prev",
                },
...
do
    ...
    config.ziggy_path = os.getenv("MPV_UOSC_ZIGGY")
        or options.ziggy_path == "default" and join_path(mp.get_script_directory(), join_path("bin", bin))
        or utils.join_path(mp.command_native({ "expand-path", options.ziggy_path }), bin)
end
...
bind_command("flash-progress", function()
    Elements:flash({ "progress" })
end)
bind_command("toggle-progress", function()
    Elements:maybe("timeline", "toggle_progress")
end)
bind_command("toggle-title", function()
    Elements:maybe("top_bar", "toggle_title")
end)
bind_command("decide-pause-indicator", function()
    Elements:maybe("pause_indicator", "decide")
end)
...
bind_command(
    "load-subtitles",
    create_track_loader_menu_opener({
        name = "subtitles",
        prop = "sub",
...
bind_command(
    "subtitles",
    create_select_tracklist_type_menu_opener(
        t("Subtitles"),
        "sub",
        "sid",
        "script-binding uosc/load-subtitles",
        "script-binding uosc/download-subtitles"
    )
)
bind_command(
    "audio",
    create_select_tracklist_type_menu_opener(t("Audio"), "audio", "aid", "script-binding uosc/load-audio")
)
bind_command(
    "video",
    create_select_tracklist_type_menu_opener(t("Video"), "video", "vid", "script-binding uosc/load-video")
)
bind_command(
    "playlist",
    create_self_updating_menu_opener({
        ...
        serializer = function(playlist)
            ...
            for index, item in ipairs(playlist) do
                ...
                items[index] = {
                    title = is_url and (item_title or playlist_titles[item.filename] or url_decode(item.filename))
                        or serialize_path(item.filename).basename,
                    hint = tostring(index),
                    active = item.current,
                    value = index,
                }
...
bind_command("delete-file-prev", function()
    delete_file_navigate(-1)
end)
...
bind_command("update", function()
    if not Elements:has("updater") then
        require("elements/Updater"):new()
    end
end)
...
mp.register_script_message("toggle-elements", function(elements)
    Elements:toggle(comma_split(elements))
end)
...
mp.register_script_message("flash-elements", function(elements)
    Elements:flash(comma_split(elements))
end)
mp.register_script_message("overwrite-binding", function(name, command)
    key_binding_overwrites[name] = command
end)
mp.register_script_message("disable-elements", function(id, elements)
    Manager:disable(id, elements)
end)
...
function Manager:_commit()
    ...
    for _, id in ipairs(self._ids) do
        ...
        if not self.disabled[id] then
            if not Elements:has(id) and constructor then
                constructor:new()
            end
        else
