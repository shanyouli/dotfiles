...
local function predict_audio()
	...
	elseif opt ~= "auto" then
		return audio_tracks[tonumber(opt)]
	end
...
local function preload()
	...
	if o.force_prediction and next(audio) then
		set_track("aid", audio.id)
	end
	select_tracks(audio)
end
...
if o.preload then
	...
	if
		o.detect_incorrect_predictions
		and not o.select_audio
		and not o.force_prediction
		and not o.observe_audio_switches
	then
