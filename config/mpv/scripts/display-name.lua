...
local function get_temp_file_name()
    ...
    if not PLATFORM_WINDOWS then
        ...
        return utils.join_path(os.getenv("TEMP"), file)
    end
end

-- creates an iterator for cells in a csv row
local function csv_iter(str)
    str = str:gsub('".-"', function(substr)
        return substr:gsub(", ", "x"):gsub(",", " "):sub(2, -2)
    end)
...
local function load_display_info()
    ...
    if not f then
        return msg.error("failed to open file ", name)
    end
    ...
    for row in f:lines() do
        ...
        if not display.Name then
            return msg.error("display did not return a name")
        end
