...
function string:split(separator)
    ...
    local copy = self:gsub(pattern, function(c)
        fields[#fields + 1] = c
    end)
    return fields
end
    ...
    for i = 1, #command do
        ...
        elseif table.has(quotation, char) and not table.has(escape, temp:sub(#temp, #temp)) then
            ...
            if quotation_stack[#quotation_stack] == char then
                quotation_stack = table.filter(quotation_stack, function(i, v)
                    return i ~= #quotation_stack
                end)
            else
...
function InputEvent:evaluate(event)
    ...
    if not actions or table.isEmpty(actions) then
        return
    end
...
local function cmd_filter(i, v)
    return (v.cmd ~= nil and v.cmd ~= "ignore")
end
...
function InputEvent:emit(event)
    ...
    if
        ...
        and (
            self.on["release"] == nil
            or table.isEmpty(self.on["release"])
            or table.isEmpty(table.filter(self.on["release"], cmd_filter))
        )
    then
        event = "release-auto"
    end
...
function InputEvent:bind()
    self.exec_debounced = debounce(function()
        self:exec()
    end, self.duration)
