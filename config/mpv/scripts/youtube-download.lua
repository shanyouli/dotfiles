...
if not_empty(opts.download_path) and utils.readdir(opts.download_path) == nil then
	...
	if res.status ~= 0 then
		msg.error(
			"Failed to create youtube-download save directory "
				.. opts.download_path
				.. ". Error: "
...
local function download(download_type, config_file, overwrite_opts)
	...
	if url:find("ytdl://") ~= 1 and url:find("https?://") ~= 1 then
	...
	elseif
		select_range_mode == 0
		or (select_range_mode > 0 and (download_type == DOWNLOAD.AUDIO or download_type == DOWNLOAD.SUBTITLE))
	then
		table.insert(command, opts.youtube_dl_exe)
		...
		table.insert(command, url)
	elseif
		select_range_mode > 0 and (download_type == DOWNLOAD.VIDEO or download_type == DOWNLOAD.VIDEO_EMBED_SUBTITLE)
	then
		...
		elseif requested_format:find("mp4") ~= nil then
			...
			msg.warn(
				'Select range mode requires a .mp4 format or "best", found '
					.. requested_format
					.. "\n("
					.. video_format
					.. ")"
		...
		if info_status ~= 0 then
			mp.set_osd_ass(0, 0, "")
			mp.osd_message(
				"Could not retieve download stream url: status="
					.. tostring(info_status)
					.. "\n"
					.. ass0
					.. "{\\fs8} "
					.. info_stdout:gsub("\r", "")
		...
		if info_lines_N < 2 then
			mp.set_osd_ass(0, 0, "")
			mp.osd_message(
				"Could not extract download stream urls and filename from output\n"
					.. ass0
					.. "{\\fs8} "
					.. info_stdout:gsub("\r", "")
					.. "\n"
	...
	if mpv_playlist_n > 0 then
		mp.set_osd_ass(
			0,
			0,
			"{\\an9}{\\fs12}" .. tostring(mpv_playlist_i) .. "/" .. tostring(mpv_playlist_n) .. "⌛💾"
		)
	else
		mp.set_osd_ass(0, 0, "{\\an9}{\\fs12}⌛💾")
	end
	...
	local function download_ended(success, ret, error)
		...
		if opts.open_new_terminal then
			...
			if playlist_finished ~= -1 then
				mp.osd_message(
					"Started last download of mpv playlist (" .. tostring(playlist_finished) .. " entries)",
					5
				)
		...
		if status == 0 and range_mode_file_name ~= nil then
			...
			command = {
				...
				"-y",
				"-i",
				range_mode_file_name,
				"-ss",
				"00:00:" .. start_time_offset_str,
				"-c",
				"copy",
				"-avoid_negative_ts",
				"make_zero",
	...
	if opts.open_new_terminal then
		...
		if has_cwd then
			...
			if
				cwd:lower():sub(1, #win_programs) == win_programs:lower()
				or cwd:lower():sub(1, #win_win) == win_win:lower()
			then
				msg.debug(
					"The mpv working directory ('" .. cwd .. "') is probably not writable. Trying %USERPROFILE%..."
				)
	...
	if select_range_mode > 0 then
		if select_range_mode == 2 then
			status = "Download range: Fine tune\n← → start time\n↓ ↑ end time\n"
				.. tostring(opts.select_range_binding)
				.. " next mode"
...
local function create_menu_data()
	...
	local items = {
		...
		},
		{
			...
			value = menu_command(
				"set-state-bool mpv_playlist_toggle " .. (switches.mpv_playlist_toggle and "no" or "yes")
			),
		},
	}
