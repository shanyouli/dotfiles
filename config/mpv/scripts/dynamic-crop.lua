...
for k, v in pairs(options) do
	...
	if timer then
		options[k] = shifting_to(RIGHT, v)
	end
end
options.read_ahead_timer = options.read_ahead_mode == 1 and options.fast_change_timer
	or options.read_ahead_mode == 2 and options.ratio_timer * (1 + options.segmentation)
	or nil
...
local function print_debug(msg_type, meta, label)
	...
	elseif msg_type == "metadata" then
		mp.msg.info(
			string.format(
				"%s, %-29s | offX:%3s offY:%3s | limit:%-2s",
				label,
				meta.whxy,
				meta.offset.x,
				meta.offset.y,
	...
	elseif msg_type == "buffer" and s.stats.buffer then
		...
		for whxy, ref in pairs(s.stats.buffer) do
			mp.msg.info(
				string.format(
					"\\ %-29s | offX=%4s offY=%4s | time=%6ss linked_source=%-4s known_ratio=%-4s trusted_offsets=%s",
					whxy,
					ref.offset.x,
					ref.offset.y,
					shifting_to(LEFT, ref.time.buffer),
					ref.is_linked_to_source or false,
					ref.is_known_ratio or false,
...
local function print_stats()
	if not s.stats and not s.stats.trusted then
		return
	end
	mp.msg.info("Meta Stats:")
	local offsets_list = { x = "", y = "" }
	for axis, _ in pairs(offsets_list) do
		for _, v in pairs(s.stats.trusted_offset[axis]) do
			offsets_list[axis] = offsets_list[axis] .. v .. " "
		end
	end
	mp.msg.info(
	...
	mp.msg.info(
		string.format("Trusted - unique: %s | offset: X:%sY:%s", s.stats.trusted_unique, offsets_list.x, offsets_list.y)
	)
	for whxy, ref in pairs(s.stats.trusted) do
		if s.stats.trusted[whxy] then
			mp.msg.info(
				string.format(
					"\\ %-29s | offX=%3s offY=%3s | applied=%s overall=%ss accumulated=%ss",
					whxy,
					ref.offset.x,
					ref.offset.y,
					ref.applied,
					shifting_to(LEFT, ref.time.overall),
	...
	mp.msg.info(
		"Buffer - unique: " .. s.stats.buffer_unique .. " | total: " .. s.buffer.i_total,
		shifting_to(LEFT, s.buffer.t_total) .. "s | known_ratio:",
		s.buffer.i_ratio,
...
local function is_trusted_offset(offset, axis)
	...
	for _, v in ipairs(trusted_offset) do
		if math.abs(offset - v) <= 1 then
			return true
		end
	end
	return false
...
local function filter_state(label, key, value)
	...
	for _, filter in pairs(filters) do
		if
			...
			and (
				not key
				or key ~= "graph" and filter[key] == value
				or key == "graph" and string.find(filter.params.graph, value)
			)
		then
			return true
		end
...
local function insert_cropdetect_filter(limit, change)
	if s.toggled > 1 or s.paused then
		return
	end
	local function insert_filter()
		...
		elseif s.f_limit_runtime and options.read_ahead_mode > 0 then
			return mp.commandv(
				...
				"pre",
				string.format(
					"@%s:lavfi=[split[a][b];[b]setpts=PTS-%s/TB,%s[b];%s]",
					labels.cropdetect,
					options.read_ahead_cropdetect,
					cropdetect,
					s.f_sync
				)
			)
			...
			return mp.commandv(
				"vf",
				"pre",
				string.format("@%s:lavfi=[split[a][b];[b]%s,nullsink;[a]null]", labels.cropdetect, cropdetect)
			)
		end
	end
...
local function apply_crop(ref, pts)
	...
	if prop_fullscreen ~= "yes" and options.fix_windowed_behavior ~= 0 then
		...
		if prop_maximized == "yes" then
			...
			if options.fix_windowed_behavior ~= 0 then
				mp.set_property("auto-window-resize", "yes")
			end
	...
	elseif filter_state(labels.crop) and not s.seeking then
		for _, axis in ipairs({ "w", "x", "h", "y" }) do -- "w""x" then "h""y" to reduce visual glitch
			if s.applied[axis] ~= ref[axis] then
				command_filter(labels.crop, axis, ref[axis], "crop")
			end
		end
	else
	...
	if options.debug and pts then
		table.insert(s.stats.indexed_applied, { ref = ref, pts = pts })
	end
end
...
local function generate_ratios(list)
	for ratio in string.gmatch(list, "%S+%s?") do
		for a, b in string.gmatch(tostring(ratio), "(%d+)/(%d+)") do
			ratio = a / b
		end
		...
		for k, v in pairs({ w = w, h = h }) do
			if v < s.source[k] - options.linked_tolerance then
				if v % 2 == 1 then
					...
					s.ratios[k][v] = true
					if margin > 0 then
						s.ratios[k][v + margin], s.ratios[k][v - margin] = true, true
					end
				end
			end
		end
...
local function switch_hwdec(id, hwdec, error)
	...
	if
		...
		and hwdec ~= s.hwdec
		and s.hwdec ~= "no"
		and not string.match(s.hwdec, "-copy")
		and filter_state(labels.cropdetect)
	then
		mp.commandv("vf", "remove", string.format("@%s", labels.cropdetect))
	end
	s.hwdec = hwdec
end
...
local function process_metadata(collected, timestamp, elapsed_time)
	...
	if s.buffer.i_total == 0 or s.buffer.indexed_list[s.buffer.i_total].ref ~= collected then
		...
		if options.read_ahead_mode > 0 then
			table.insert(collected.pts, timestamp)
		end
	...
	if s.buffer.i_ratio > 0 then
		s.buffer.t_ratio = s.buffer.t_ratio + elapsed_time
	end

	-- candidate offset/fallback to later extend buffer size
	if
		not s.stats.trusted[collected.whxy]
		and collected.time.buffer > options.ratio_timer
		and collected.is_linked_to_source
	then
		...
		elseif
			not collected.is_known_ratio
			and not s.candidate.offset[collected.whxy]
			and not s.candidate.fallback[collected.whxy]
		then
	...
	if
		s.candidate.offset[collected.whxy]
		and collected.is_known_ratio
		and collected.is_linked_to_source
		and collected.time.buffer >= options.offset_timer
	then
	...
	local new_ready = not s.stats.trusted[collected.whxy]
		and collected.is_trusted_offsets
		and not collected.is_invalid
		and collected.is_linked_to_source
		and collected.is_known_ratio
		and collected.time.buffer >= options.ratio_timer
	if new_ready then
	...
	if not current.is_invalid and s.stats.trusted_unique > 1 and not s.stats.trusted[current.whxy] then
		...
		corrected.is_bigger = current.mt < s.approved.mt
			or current.mb < s.approved.mb
			or current.ml < s.approved.ml
			or current.mr < s.approved.mr
		...
		for _, ref in pairs(s.stats.trusted) do
			...
			for _, side in ipairs({ "mt", "mb", "ml", "mr" }) do
				...
				if diff[side] > margin or diff[side] < -margin then
					diff.vs_current = diff.vs_current + 1
				end
				if ref[side] ~= s.approved[side] then
			...
			local pattern = diff.is_inside
				and (
					diff.vs_current <= 1
					or diff.vs_current == 2 and diff.vs_applied <= 2
					or diff.vs_current > 2 and corrected.is_bigger
				)
			local set = closest.ref
				and (
					diff.vs_current < closest.vs_current
					or diff.vs_current == closest.vs_current and diff.vs_applied < closest.vs_applied
					or diff.vs_current == closest.vs_current
						and diff.vs_applied == closest.vs_applied
						and diff.total < closest.total
				)
	...
	if options.detect_round <= 4 and s.stats.trusted[current.whxy] then
		...
		local applied_in_margin = math.abs(current.w - s.approved.w) <= margin
			and math.abs(current.h - s.approved.h) <= margin
		for _, ref in pairs(s.stats.trusted) do
			local in_margin = math.abs(current.w - ref.w) <= margin and math.abs(current.h - ref.h) <= margin
			if in_margin then
				local gt_applied = applied_in_margin
					and ref ~= s.approved
					and ref.time.overall > s.approved.time.overall * 2
				local applied_gt = applied_in_margin
					and ref == s.approved
					and ref.time.overall * 2 > current.time.overall
				local pattern = not applied_in_margin and ref.time.overall > current.time.overall
					or gt_applied
					or applied_gt
				...
				if ref ~= current and pattern and (not stabilized or set) then
					stabilized = ref
				end
			end
		end
	...
	for whxy, ref in pairs(s.stats.trusted) do
		ref.time.accumulated = whxy ~= current.whxy and 0
			or ref.time.accumulated < 0 and 0 + elapsed_time
			or not new_ready and ref.time.accumulated + elapsed_time
			or ref.time.accumulated
	end

	-- crop: final validation then store or apply it
	local detect_source = current == s.last_current
		and (current.is_source or collected.is_source)
		and s.limit.target >= 0
	local confirmation = not current.is_source
		and s.stats.trusted[current.whxy]
		and current.time.accumulated >= options.fast_change_timer
		and (not corrected.ref or current == s.last_current)
	...
	if crop_filter and (not s.timestamps.prevent or timestamp >= s.timestamps.prevent) then
		...
		if s.f_limit_runtime and options.read_ahead_mode > 0 then
			local pts = current.time.accumulated < options.ratio_timer and timestamp - current.time.accumulated
				or current.pts[1]
		...
		if options.prevent_change_mode > 0 then
			...
			if
				options.prevent_change_mode == 1 and (current.w > s.approved.w or current.h > s.approved.h)
				or options.prevent_change_mode == 2 and (current.w < s.approved.w or current.h < s.approved.h)
				or options.prevent_change_mode == 3
			then
	...
	local buffer_timer = s.candidate.i_offset > 0 and options.offset_timer
		or s.candidate.i_fallback > 0 and options.fallback_timer
		or options.ratio_timer
	...
	if s.buffer.i_to_shift >= 20 or s.buffer.i_to_shift == s.buffer.i_total then
		...
		for i = 0, s.buffer.i_to_shift - 1 do
			s.buffer.indexed_list[s.buffer.i_total - i] = nil
		end
	...
	if s.f_limit_runtime or timestamp >= s.limit.timer then
		...
		elseif
			not current.is_invalid
			and (collected.is_trusted_offsets or collected == s.last_collected or current == s.last_current)
		then
			-- stable limit
			s.limit.target = 0
	...
	if s.last_limit ~= s.limit.current then
		if not s.f_limit_runtime and options.limit_timer > 0 then
			s.limit.timer = timestamp + options.limit_timer
		end
...
local function collect_metadata(event)
	if
		event.prefix == "ffmpeg"
		and event.level == "v"
		and string.find(event.text, "^.*dyn_cd: ")
		and not (s.seeking or s.paused or s.toggled > 1)
	then
		local tmp = {}
		for k, v in string.gmatch(event.text, "(%w+):(%-?%d+%.?%d* )") do
			tmp[k] = tonumber(v)
		end
		...
		if not wait_limit then
			s.f_limit_change = false
		end

		if
			s.in_progress
			or not s.timestamps.previous
			or wait_limit
			or s.f_inserted
			or s.timestamps.current < options.start_delay
		then
			s.f_inserted = false
			return
		end
		...
		if not s.frametime or elapsed_time < s.frametime and elapsed_time > 0 then
			s.frametime = elapsed_time
		end
	...
	if event == "seek" or event == "toggle" then
		...
		if event == "seek" then
			...
			if
				not s.f_video_crop
				and (
					filter_state(labels.crop, "enabled", true)
					or not filter_state(labels.crop) and s.applied ~= s.source
				)
			then
...
function on_toggle(auto)
	...
	if s.toggled == ENABLE then
		...
		seek(EVENT)
		if not auto then
			mp.osd_message(string.format("%s: disabled, crop remains.", label_prefix), 3)
		end
	elseif s.toggled == DISABLE_WITH_CROP then
		...
		if not auto then
			mp.osd_message(string.format("%s: crop removed.", label_prefix), 3)
		end
		...
		resume(EVENT)
		if not auto then
			mp.osd_message(string.format("%s: enabled.", label_prefix), 3)
		end
	end
end
...
function cleanup()
	...
	for _, label in pairs(labels) do
		if filter_state(label) then
			mp.commandv("vf", "remove", string.format("@%s", label))
		end
	end
	if s.f_video_crop then
		mp.set_property("video-crop", "")
...
local function on_start()
	...
	if options.read_ahead_mode > 0 then
		...
		s.f_sync = mp.commandv("vf", "add", string.format("@%s:lavfi=[split[a][b];[a][b]dummysync]", label_prefix))
				and mp.commandv("vf", "remove", string.format("@%s", label_prefix))
				and "[a][b]dummysync"
			or "[a][b]psnr=eof_action=pass"
	end
