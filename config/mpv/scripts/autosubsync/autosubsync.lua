...
local function esc_for_title(string)
	string = string:gsub("^[%._%-%s]*", ""):gsub("%.%w+$", "")
	return string
end

local function esc_for_code(trackCode)
	if trackCode:find("PGS") then
		trackCode = "PGS"
	elseif trackCode:find("SUBRIP") then
		trackCode = "SRT"
	elseif trackCode:find("VTT") then
		trackCode = "VTT"
	elseif trackCode:find("DVD_SUB") then
		trackCode = "VOB_SUB"
	elseif trackCode:find("DVB_SUB") then
		trackCode = "DVB_SUB"
	elseif trackCode:find("DVB_TELE") then
		trackCode = "TELETEXT"
	elseif trackCode:find("ARIB") then
		trackCode = "ARIB"
	end
	return trackCode
end
		...
		if not track.selected and supported_format then
			...
			table.insert(
				...
				string.format(
					...
					track["id"],
					(track.lang or (track.title and esc_for_title(replace(track.title, filename, "")) or "unknown")),
