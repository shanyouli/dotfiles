...
function SRT:populate(filename)
	...
	for _, line in pairs(self:parse_file(filename)) do
		if idx == 1 and #line > 0 then
			assert(
				line:match("^%d+$"),
				string.format("SRT FORMAT ERROR (line %d): expected a number but got '%s'", f_idx, line)
			)
			entry.index = line
		elseif idx == 2 then
			assert(
...
function ASS:toString()
	...
	for i = 1, #self.entries do
		if i == 1 then
			...
			local event_sb = {}
			for _, v in pairs(self.event_header) do
				table.insert(event_sb, v)
			end
...
function ASS:populate(filename, language)
	...
	local columns = {}
	for i in colstring:gmatch("[^%,%s]+") do
