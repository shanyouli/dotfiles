        ...
        if subtitle.stop >= start or same_time(subtitle.stop, start) then
            for i = #lines, 1, -1 do
                if lines[i] == subtitle.line then
                    ...
                    if start < subtitle.start then
                        subtitle.start = start
                    end
                    if stop > subtitle.stop then
                        subtitle.stop = stop
                    end
                end
            end
...
local function fix_line_timing(prev_subs_visible, lines, start, prev_start)
    ...
    if start == prev_start then
        local start_approx = mp.get_property_number("time-pos", 0)
            - mp.get_property_number(sub_strings.delay)
            - SUB_SEEK_OFFSET
...
local function show_subtitle_list(subtitles)
    ...
    menu.selected_index = last_active_index
        or last_started_index and subtitles[last_started_index + 1] and last_started_index + 1
        or last_started_index
