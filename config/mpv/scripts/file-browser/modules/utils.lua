...
function fb_utils.valid_file(file)
    ...
    if o.filter_files and not g.extensions[fb_utils.get_extension(file, "")] then
        return false
    end
    return true
end
...
function fb_utils.filter(t)
    ...
    for i = 1, max do
        ...
        if
            (temp.type == "dir" and fb_utils.valid_dir(temp.label or temp.name))
            or (temp.type == "file" and fb_utils.valid_file(temp.label or temp.name))
        then
            t[top] = temp
            top = top + 1
        end
...
local function create_item_string(base_code_fn, items, state, cmd, quoted)
    ...
    local func = quoted and function(...)
        return ("%q"):format(base_code_fn(...))
    end or base_code_fn
...
fb_utils.code_fns = {
    ...
    f = function(item, s)
        return item and fb_utils.get_full_path(item, s.directory) or ""
    end,
    n = function(item, s)
    ...
    i = function(item, s)
        ...
        return i ~= -1 and i or 0
    end,
    j = function(item, s)
        return fb_utils.list.indexOf(s.list, item) ~= -1
                and math.abs(fb_utils.list.indexOf(fb_utils.sort_keys(s.selection), item))
            or 0
    end,
    ...
    q = function(_, s)
        return s.directory == "" and "ROOT" or s.directory_label or s.directory or ""
    end,
    d = function(_, s)
...
function fb_utils.get_code_pattern(codes)
    ...
    for key in pairs((getmetatable(codes) or {}).__index or {}) do
        CUSTOM_KEYBIND_CODES = CUSTOM_KEYBIND_CODES .. key:lower() .. key:upper()
    end
...
function fb_utils.substitute_codes(str, overrides, item, state, modifier_fn)
    local replacers = overrides and setmetatable(fb_utils.copy_table(overrides), { __index = fb_utils.code_fns })
        or fb_utils.code_fns
    item = item or g.state.list[g.state.selected]
    state = state or g.state

    return (
        string.gsub(str, fb_utils.get_code_pattern(replacers), function(code)
            local result
