...
g.state.keybinds = {
	...
	},
	{
		...
		"play_append",
		function()
			playlist.add_files("append-play", false)
		end,
	},
	{
		"Alt+ENTER",
		"play_autoload",
		function()
			playlist.add_files("replace", true)
		end,
	},
...
local function substitute_codes(str, cmd, items, state)
	...
	for code in item_specific_codes:gmatch(".") do
		overrides[code] = function(_, s)
			return create_item_string(fb_utils.code_fns[code], items, s, cmd)
		end
		overrides[code:upper()] = function(_, s)
			return create_item_string(fb_utils.code_fns[code], items, s, cmd, true)
		end
	end
...
local function has_item_codes(codes)
	for code in pairs(codes) do
		if item_specific_codes:find(code:lower(), 1, true) then
			return true
		end
	end
	return false
...
local function run_custom_keybind(cmd, state, co)
	...
	if type(cmd.command) == "function" then
		return cmd.command(cmd, cmd.addon and fb_utils.copy_table(state) or state, co)
	end
			...
			if parser.keybinds then
				for i, keybind in ipairs(parser.keybinds) do
					...
					if not keybind.key then
						...
						keybind = fb_utils.copy_table(keybind)
					end
