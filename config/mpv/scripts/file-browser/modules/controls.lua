...
function controls.open()
	...
	if o.set_shared_script_properties then
		utils.shared_script_property_set("file_browser-open", "yes")
	end
	if o.set_user_data then
		mp.set_property_bool("user-data/file_browser/open", true)
	end
	...
	if not g.state.flag_update then
		ass.draw()
	else
		g.state.flag_update = false
		ass.update_ass()
	end
end
...
function controls.close()
	...
	if o.set_user_data then
		mp.set_property_bool("user-data/file_browser/open", false)
	end
