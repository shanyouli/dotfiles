...
if history_dir ~= "" then
	...
	if not meta or not meta.is_dir then
		...
		if res.status ~= 0 then
			msg.error(
				"Failed to create history_dir save directory " .. history_dir .. ". Error: " .. (res.error or "unknown")
			)
			return
		end
	end
...
local function get_bookmark_path(dir)
	...
	if is_windows then
		bookmark_path = bookmark_path:gsub("\\", "/")
	end
end
...
local function record()
	...
	refresh_globals()
	if pl_count and pl_count < 1 then
		return
	end
	if not path or is_protocol(path) or not file_exist(path) then
		return
	end
	...
	if included_dir_count > 0 then
		if not need_ignore(o.included_dir, dir) then
			return
		end
	end
	if need_ignore(o.excluded_dir, dir) then
		return
	end
	...
	if current_idx then
		msg.verbose("current index -- " .. current_idx)
	end
