...
function list_sort(tab, sort)
	...
	elseif sort == "keybind-asc" and filterName == "keybinds" then
		table.sort(tab, function(a, b)
			return a["found_slot"] > b["found_slot"]
		end)
	...
	elseif sort == "alphanum-asc" or sort == "alphanum-desc" then
		...
		if sort == "alphanum-asc" then
			table.sort(tab, function(a, b)
				return tostring(a["found_path"]):lower():gsub("%.?%d+", padnum) .. ("%3d"):format(#b)
					> tostring(b["found_path"]):lower():gsub("%.?%d+", padnum) .. ("%3d"):format(#a)
			end)
...
function get_o_variable(str, arr_var) --1.3# function to get variable content from passed array
	...
	if str:match("%%(.*)%%") then
		str = str:match("%%(.*)%%")
	end --1.3# if the entry has % around it, then remove it
...
function parse_list_item(str, properties) --1.3#add ability to parse the contents of the list like the header
	if not str then
		return msg.error("str in parse_list_item is nil")
	end
	...
	if properties["item"].found_time and tonumber(properties["item"].found_time) > 0 then
		str = str:gsub(
			"%%duration%%",
			format_time(
				properties["item"].found_time,
				o.list_duration_time_format[3],
				o.list_duration_time_format[2],
				o.list_duration_time_format[1]
			)
		)
	...
	if properties["item"].found_length and tonumber(properties["item"].found_length) > 0 then
		str = str:gsub(
			"%%length%%",
			format_time(
				properties["item"].found_length,
				o.list_length_time_format[3],
				o.list_length_time_format[2],
				o.list_length_time_format[1]
			)
		)
	...
	if properties["item"].found_remaining and tonumber(properties["item"].found_remaining) > 0 then
		str = str:gsub(
			"%%remaining%%",
			format_time(
				properties["item"].found_remaining,
				o.list_remaining_time_format[3],
				o.list_remaining_time_format[2],
				o.list_remaining_time_format[1]
			)
		)
...
function parse_header(str)
	...
	str = str:gsub("%%total%%", #osd_log_contents):gsub("%%cursor%%", list_cursor)

	local filter_osd = filterName
	if filter_osd ~= "all" then
		if filter_osd:match("/:group%%(.*)%%") then
			filter_osd = filter_osd:match("/:group%%(.*)%%")
		end
	...
	if str:match("%%length%%") then
		if get_total_duration("found_length") > 0 then
			str = str:gsub(
				"%%length%%",
				format_time(
					get_total_duration("found_length"),
	...
	if str:match("%remaining%%") then
		if get_total_duration("found_remaining") > 0 then
			str = str:gsub(
				"%%remaining%%",
				format_time(
					get_total_duration("found_remaining"),
...
function search_log_contents(arr_contents)
	if not arr_contents or not arr_contents[1] or not search_active or not search_string == "" then
		return false
	end
		...
		if o.search_behavior == "any" then
			...
			if parse_8601(osd_log_contents[i].found_datetime) then --1.3# an if statement to check if date could be parsed --1.3# allows for all type of dates to be searched thanks to the loop
				local os_date_tag = {
					...
					"%Y",
					"%y",
				} --1.3# add all lua date time parameters
				for j = 1, #os_date_tag do --1.3# replace all lua parameters with date values that can be searched
					contents_string = contents_string
						.. os.date(os_date_tag[j], parse_8601(osd_log_contents[i].found_datetime))
				end
			end
			contents_string = contents_string
				.. (osd_log_contents[i].found_title or "")
				.. (osd_log_contents[i].found_name or "")
				.. osd_log_contents[i].found_path --1.3# added found_name since parsing is different now
			if tonumber(osd_log_contents[i].found_time) > 0 then
				contents_string = contents_string
					.. format_time(
						osd_log_contents[i].found_time,
						o.list_duration_time_format[3],
						o.list_duration_time_format[2],
						o.list_duration_time_format[1]
					)
			...
			if tonumber(osd_log_contents[i].found_length) > 0 then
				contents_string = contents_string
					.. format_time(
						osd_log_contents[i].found_length,
						o.list_length_time_format[3],
						o.list_length_time_format[2],
						o.list_length_time_format[1]
					)
			...
			if tonumber(osd_log_contents[i].found_remaining) > 0 then
				contents_string = contents_string
					.. format_time(
						osd_log_contents[i].found_remaining,
						o.list_remaining_time_format[3],
						o.list_remaining_time_format[2],
						o.list_remaining_time_format[1]
					)
			...
			if osd_log_contents[i].found_group then
				contents_string = contents_string
					.. get_group_properties(tonumber(osd_log_contents[i].found_group)).name
			end
...
function filter_log_contents(arr_contents, filter)
	if not arr_contents or not arr_contents[1] or not filter or filter == "all" then
		return false
	end
	local filtered_arr_contents = {}

	if filter:match("%%%+%%") then
		if filter_stack(arr_contents, filter) then
			filtered_arr_contents = filter_stack(arr_contents, filter)
		end
	elseif filter:match("%%%-%%") then
		if filter_omit(arr_contents, filter) then
			filtered_arr_contents = filter_omit(arr_contents, filter)
		end
	else
		if filter_apply(arr_contents, filter) then
			filtered_arr_contents = filter_apply(arr_contents, filter)
		end
	end
...
function filter_omit(arr_contents, filter)
	if not arr_contents or not arr_contents[1] or not filter or filter == "all" or not filter:match("%%%-%%") then
		return false
	end
	...
	for i = 1, #filter_items do
		if i == 1 and filter_apply(arr_contents, filter_items[i]) then
			omitted_arr_table = filter_apply(arr_contents, filter_items[i])
		end
		if i > 1 then
			if filter_apply(arr_contents, filter_items[i]) then
				temp_filtered_contents = filter_apply(arr_contents, filter_items[i])
			end
			for j = 1, #temp_filtered_contents do
				for k = 1, #omitted_arr_table do
					if
						temp_filtered_contents[j]
						and omitted_arr_table[k]
						and temp_filtered_contents[j].found_sequence == omitted_arr_table[k].found_sequence
					then
	...
	table.sort(omitted_arr_table, function(a, b)
		return a["found_sequence"] < b["found_sequence"]
	end)
...
function filter_stack(arr_contents, filter)
	...
	table.sort(stacked_arr_table, function(a, b)
		return a["found_sequence"] < b["found_sequence"]
	end)
...
function filter_apply(arr_contents, filter)
	...
	if filter:match("/:group%%(.*)%%") then
		...
		for i = 1, #arr_contents do
			if
				arr_contents[i].found_group
				and filter == get_group_properties(tonumber(arr_contents[i].found_group)).name
			then
				table.insert(filtered_arr_contents, arr_contents[i])
			end
		end
	...
	if filter == "recents" then
		table.sort(arr_contents, function(a, b)
			return a["found_sequence"] < b["found_sequence"]
		end)
		local unique_values = {}
		...
		if
			filePath == arr_contents[#arr_contents].found_path
			and tonumber(arr_contents[#arr_contents].found_time) == 0
		then
		...
		table.sort(filtered_arr_contents, function(a, b)
			return a["found_sequence"] < b["found_sequence"]
		end)
	end
	...
	if filter == "distinct" then
		...
		for i = list_total, 1, -1 do
			if
				arr_contents[i].found_directory
				and not has_value(unique_values, arr_contents[i].found_directory)
				and not starts_protocol(protocols, arr_contents[i].found_path)
			then
...
function get_osd_log_contents(filter, sort)
	...
	if filter_log_contents(osd_log_contents, filter) then
		osd_log_contents = filter_log_contents(osd_log_contents, filter)
	end
	if search_log_contents(osd_log_contents) then
		osd_log_contents = search_log_contents(osd_log_contents)
	end
...
function parse_8601(timestamp)
	if not string.match(timestamp, "^(%d%d%d%d)-(%d%d)-(%d%d)T(%d%d):(%d%d):(%d%d)(.-)$") then
		return false
	end
	local inYear, inMonth, inDay, inHour, inMinute, inSecond, inZone =
		string.match(timestamp, "^(%d%d%d%d)-(%d%d)-(%d%d)T(%d%d):(%d%d):(%d%d)(.-)$")
...
function display_list(filter, sort, action)
	...
	if not osd_log_contents and not search_active or not osd_log_contents[1] and not search_active then
		return
	end
	...
	if list_cursor_temp > 0 and list_cursor_temp <= #osd_log_contents then
		...
		if action == "highlight" then
			if not has_value(list_highlight_cursor, list_cursor, 1) then
					...
					for i = pos, 1, -1 do
						if not has_value(list_highlight_cursor, list_cursor - i, 1) then
							table.insert(
								...
								{ list_cursor - i, osd_log_contents[#osd_log_contents + 1 + i - list_cursor] }
							)
						end
					end
				else
				...
				table.insert(
					list_highlight_cursor,
					{ list_cursor, osd_log_contents[#osd_log_contents + 1 - list_cursor] }
				)
			else
...
function load(list_cursor, add_playlist, target_time)
	...
	if not target_time then
		if not osd_log_contents[#osd_log_contents - list_cursor + 1] then
			return
		end --1.3.0# fixes crash when loading an entry that doesn't exist
	...
	if
		file_exists(osd_log_contents[#osd_log_contents - list_cursor + 1].found_path)
		or starts_protocol(protocols, osd_log_contents[#osd_log_contents - list_cursor + 1].found_path)
	then
		local list_filename, list_filepath, list_filetitle =
			get_local_names(osd_log_contents[#osd_log_contents - list_cursor + 1]) --1.3# use the name that automatically falls back instead for osd printing or msg (solves the issue that causes concatinating found_name to crash because it sometimes doesn't exist due to parsing changes)
...
function same_path_log_delete(target_path, entry_limit, arr_contents)
	...
	if not target_path then
		return msg.error("same_path_log_delete no target_path defined")
	end
	if not arr_contents then --1.2.6# ability to pass array (usually automatically defining array here is fine, but just for performance sake when calling multiple functions that use the same array)
		...
		if not arr_contents or not arr_contents[1] then
			return
		end
	end
...
function find_entry(round, target_path, target_time) --1.2.6# changed it to find_entry to have the sequence and any other additional property
	...
	if not target_path or not target_time then
		return msg.error("find_entry no target_path or target_time defined")
	end
	local temp_log_contents = read_log_table()
	if not temp_log_contents or not temp_log_contents[1] then
		return
	end
	...
	for i = #temp_log_contents, 1, -1 do
		if not round then
			if
				temp_log_contents[i].found_path == target_path
				and tonumber(temp_log_contents[i].found_time) == target_time
			then
			...
			if
				temp_log_contents[i].found_path == target_path
				and math.floor(tonumber(temp_log_contents[i].found_time)) == target_time
			then
				return temp_log_contents[i]
			end
		end
...
function delete_log_entry(target_sequence, arr_contents)
	...
	if not target_sequence then
		return
	end --1.2.5# if no sequence found then just exit the function
	...
	for i = 1, log_contents_length do
		for j = 1, #list_highlight_cursor do
			if temp_log_contents[log_contents_length + 1 - i] then
				if
					temp_log_contents[log_contents_length + 1 - i].found_sequence
...
function update_list_highlist_cursor()
	...
	for i = 1, #osd_log_contents do
		for j = 1, #list_highlight_cursor do
			if
				osd_log_contents[#osd_log_contents + 1 - i].found_sequence == list_highlight_cursor[j][2].found_sequence
			then
...
function get_list_keybinds()
	...
	bind_keys(o.keybinds_remove_highlighted_keybind, "keybind-slot-remove-highlight", function()
		slot_remove("highlight")
	end)
	bind_keys(o.list_group_add_cycle_keybind, "group-add-cycle", list_group_add_cycle)
	bind_keys(o.list_group_add_cycle_highlighted_keybind, "group-add-cycle-highlight", function()
		list_group_add_cycle("highlight")
	end)
	bind_keys(o.list_groups_remove_keybind, "group-remove", group_remove)
	bind_keys(o.list_groups_remove_highlighted_keybind, "group-remove-highlight", function()
		group_remove("highlight")
	end)

	for i = 1, #o.groups_list_and_keybind do
		if not o.groups_list_and_keybind[i][2] then
			break
		end
		mp.add_forced_key_binding(o.groups_list_and_keybind[i][2], "group-add-" .. i, function()
			group_add(i)
		end)
	end
	for i = 1, #o.groups_list_and_keybind do
		if not o.groups_list_and_keybind[i][3] then
			break
		end
		mp.add_forced_key_binding(o.groups_list_and_keybind[i][3], "group-add-highlight-" .. i, function()
			group_add(i, "highlight")
		end)
	end
...
function bind_search_keys()
	...
	if o.search_not_typing_smartly then
		...
		bind_keys(o.keybinds_remove_keybind, "keybind-slot-remove", function()
			slot_remove()
			list_search_not_typing_mode(true)
		end)
		bind_keys(o.keybinds_remove_keybind, "keybind-slot-remove-highlight", function()
			slot_remove("highlight")
			list_search_not_typing_mode(true)
		end)
	end
...
function remove_all_additional_param_log_entry(index, log_text)
	if not index or not log_text then
		return
	end
	local temp_log_contents = read_log_table()
	if not temp_log_contents or not temp_log_contents[1] then
	...
	for i = #temp_log_contents, 1, -1 do
		if temp_log_contents[i].found_line:find(log_text .. index) then
			temp_log_contents[i].found_line =
				string.gsub(temp_log_contents[i].found_line, " | " .. log_text .. index, "")
		end
	end
...
function remove_additional_param_log_entry(index, target, log_text)
	if not index or not target or not log_text then
		return msg.error("remove_additional_param_log_entry parameters not defined")
	end
	if not osd_log_contents or not osd_log_contents[1] then
		return
	end
	...
	if temp_log_contents[log_index].found_line:find(log_text .. index) then
		temp_log_contents[log_index].found_line =
			string.gsub(temp_log_contents[log_index].found_line, " | " .. log_text .. index, "")
	else
...
function add_additional_param_log_entry(index, target, log_text)
	if not index or not target or not log_text then
		return msg.error("add_additional_param_log_entry parameters not defined")
	end
	if not osd_log_contents or not osd_log_contents[1] then
		return
	end
	...
	if temp_log_contents[log_index].found_line then
		if temp_log_contents[log_index].found_line:sub(-1) ~= " " then
			temp_log_contents[log_index].found_line = temp_log_contents[log_index].found_line
				.. " | "
				.. log_text
				.. index
				.. " | "
...
function list_slot_remove(index, action)
	...
	if not index then
		index = tonumber(osd_log_contents[#osd_log_contents - list_cursor + 1].found_slot)
	end
	...
	if action ~= "silent" then
		msg.info("Removed Keybind: " .. get_slot_keybind(index))
	end
end
	...
	for i = 1, #osd_log_contents do
		for j = 1, #list_highlight_cursor do
			if osd_log_contents[#osd_log_contents + 1 - i] then
				if
					osd_log_contents[#osd_log_contents + 1 - i].found_sequence
					== list_highlight_cursor[j][2].found_sequence
				then
...
function list_slot_add(index)
	...
	local cursor_filename, cursor_filepath, cursor_filetitle =
		get_local_names(osd_log_contents[#osd_log_contents - list_cursor + 1]) --1.2.4# added the new name calling method to fix issue of unable to add urls into groups or slots
	...
	msg.info(
		"Added Keybind:\n"
			.. cursor_filetitle
			.. " 🕒 "
			.. format_time(cursor_seektime)
			.. " ⌨ "
			.. get_slot_keybind(index)
	)
end
...
function list_group_remove(action)
	...
	if action ~= "silent" then
		msg.info("Removed Group: " .. get_group_properties(groupCursorIndex).name)
	end
end
...
function list_group_add(index)
	...
	msg.info(
		"Added Group:\n"
			...
			.. format_time(cursor_seektime)
			.. " 🖿 "
			.. get_group_properties(index).name
	)
end
...
function mark_chapter()
	if not o.mark_bookmark_as_chapter then
		return
	end
...
function write_log(target_time, update_seekTime, entry_limit)
	...
	elseif
		o.file_title_logging == "protocols" and (starts_protocol(o.logging_protocols, filePath))
		or o.file_title_logging == "local" and not (starts_protocol(o.logging_protocols, filePath))
	then --1.3# added file_title_logging for local
		f:write(
			('[%s] "%s" | %s | %s | %s | '):format(
				os.date("%Y-%m-%dT%H:%M:%S"),
				fileTitle,
...
function add_load_slot(key_index)
	...
	if list_drawn then
		...
		if osd_log_contents ~= nil and osd_log_contents[1] then
			...
			if slot_taken then
				...
				if o.keybinds_empty_auto_create then
					if filePath ~= nil then
						...
						msg.info(
							"Bookmarked the below & added keybind:\n"
								.. fileTitle
								.. " 🕒 "
								.. format_time(mp.get_property_number("time-pos"))
								.. " ⌨ "
								.. get_slot_keybind(key_index)
						)
					else
					...
					msg.info(
						"No bookmark slot has been assigned for"
							.. " ⌨ "
							.. get_slot_keybind(key_index)
							.. " keybind yet"
					)
			...
			msg.info(
				"No bookmark slot has been assigned for" .. " ⌨ " .. get_slot_keybind(key_index) .. " keybind yet"
			)
		end
	end
end
...
mp.observe_property("idle-active", "bool", function(_, v)
	...
	if v and type(o.load_item_on_startup) == "number" and not loadTriggered then --1.3.0# option to immediately load an entry based on number
		if o.load_item_on_startup == 0 then
			return
		end --1.3.0# if the entry loaded is 0 then exit this, this is automatically handled in load also but it is better to exit here since there will be a loop below this
		...
		if o.load_item_on_startup == -1 then
			o.load_item_on_startup = #osd_log_contents
		end --1.3.0# specify -1 as last entry
...
for i = 1, #o.keybinds_add_load_keybind do
	mp.add_forced_key_binding(o.keybinds_add_load_keybind[i], "keybind-slot-" .. i, function()
		add_load_slot(i)
	end)
end

for i = 1, #o.keybinds_quicksave_keybind do
	mp.add_forced_key_binding(o.keybinds_quicksave_keybind[i], "keybind-slot-save-" .. i, function()
		quicksave_slot(i)
	end)
end
