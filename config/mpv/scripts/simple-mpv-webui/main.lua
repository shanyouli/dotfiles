...
local function get_audio_devices()
    ...
    for _, device in pairs(mp.get_property_native("audio-device-list")) do
        if
            ...
            or options.audio_devices == device.name
...
]]):format(tostring(code), status_headers[code], content_type, content_length, custom_headers)
end
...
local endpoints = {
    ...
    ["api/cycle_audio_device"] = {
        POST = function(_)
            ...
            local _, success, ret =
                pcall(mp.commandv, "osd-msg", "cycle_values", "audio-device", unpack(audio_devices_list))
            return handle_post(success, ret)
        end,
    },
    ...
    ["api/loadfile"] = {
        POST = function(request)
            ...
            if mode ~= nil and mode ~= "" and mode ~= "replace" and mode ~= "append" and mode ~= "append-play" then
...
local function log_line(request, code, length)
    ...
    mp.msg.info(('%s - %s [%s] "%s" %s %s "%s" "%s"'):format(clientip, user, time, path, code, length, referer, agent))
end
...
local function init_servers()
    ...
    if options.ipv6 then
        for k, v in pairs(get_server(6)) do
            servers[k] = v
        end
    end
    if options.ipv4 then
        for k, v in pairs(get_server(4)) do
            servers[k] = v
        end
    end
