...
function track_layout_hash(tracklist)
    ...
    for _, track in ipairs(tracklist) do
        t[#t + 1] = string.format(
            ...
            track.type,
            track.id,
            tostring(track.default),
            tostring(track.external),
            track.lang or "",
            track.external and "" or (track.title or "")
        )
    end
...
function trackselect()
    ...
    if need_ignore(options.special_protocols, fpath) then
        return
    end
    ...
    for _, track in ipairs(tracklist) do
        if
            options.smart_keep
            and last[track.type] ~= nil
            and last[track.type].lang == track.lang
            and last[track.type].codec == track.codec
            and last[track.type].external == track.external
            and last[track.type].title == track.title
        then
        ...
        if
            options["preferred_" .. track.type .. "_lang"] ~= ""
            or options["excluded_" .. track.type .. "_words"] ~= ""
            or options["expected_" .. track.type .. "_words"] ~= ""
            or (options["preferred_" .. track.type .. "_channels"] or "") ~= ""
        then
            ...
            if
                next(tracks[track.type].best) == nil
                or not (tracks[track.type].best.external and tracks[track.type].best.lang ~= nil and not track.external)
            then
                if
                    options["excluded_" .. track.type .. "_words"] == ""
                    or not contains(track, options["excluded_" .. track.type .. "_words"], "title")
                then
                    if
                        options["expected_" .. track.type .. "_words"] == ""
                        or contains(track, options["expected_" .. track.type .. "_words"], "title")
                    then
                        local pass = true
                        ...
                        if options["preferred_" .. track.type .. "_lang"] ~= "" then
                            lang =
                                preferred(track, options["preferred_" .. track.type .. "_lang"], "lang", "lang_score")
                        end
                        if
                            (options["preferred_" .. track.type .. "_lang"] == "" and pass)
                            or channels
                            or lang
                            or (
                                track.external
                                and track.lang == nil
                                and (not tracks[track.type].best.external or tracks[track.type].best.lang == nil)
                            )
                        then
                            tracks[track.type].best = track
                        end
