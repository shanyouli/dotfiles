...
local osd_timer = mp.add_timeout(1, function()
    menu_close()
end)
osd_timer:kill()
...
local function download_formats(url)
    if currently_fetching[url] then
        return
    end
    ...
    if opts.ytdl_ver == "yt-dlp" then
        command[#command + 1] = "--no-match-filter"
    end
    command[#command + 1] = "--"
...
local function text_menu_open(formats, active_format, menu_type)
    ...
    if open_menu_state and open_menu_state == open_menu_state.to_menu and destructor then
        destructor()
    end
    destructor = function()
    ...
    bind_keys(opts.up_binding, "move_up", function()
        selected_move(-1)
    end, { repeatable = true })
    bind_keys(opts.down_binding, "move_down", function()
        selected_move(1)
    end, { repeatable = true })
...
local function identical_for_all(formats, properties)
    ...
    local function all_formats_same_value(formats, prop)
        ...
        for _, format in ipairs(formats) do
            ...
            if format.properties[prop] ~= first_value then
                return false
            end
        end
        return true
...
local function loading_message(menu_type)
    ...
    if uosc_available then
        if open_menu_state and open_menu_state == menu_type then
            return
        end
        local menu = {
            title = menu_type.type_capitalized .. " Formats",
...
function video_formats_toggle()
    toggle_menu(states.video_menu)
end
function audio_formats_toggle()
    toggle_menu(states.audio_menu)
end
...
mp.register_event("file-loaded", function()
    if not (opts.fetch_formats and opts.fetch_on_start) then
        return
    end
    if not current_url or url_data[current_url] then
        return
    end
    download_formats(current_url)
end)
...
mp.add_hook("on_load", 9, function()
    ...
    if not (data and data.video_active_id and data.audio_active_id) then
        return
    end
