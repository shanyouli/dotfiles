...
local function main(key, state, co)
    ...
    if key.name == "find/find" then
        text = "Find: enter search string"
    else
        text = "Find: enter advanced search string"
    end

    local query, error =
        coroutine.yield(input.get_user_input(fb.coroutine.callback(), { text = text, id = "find", replace = true }))
