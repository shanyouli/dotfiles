...
function favs:parse(directory)
	...
	if directory == "Favourites/" then
		...
		if self.cursor ~= 1 then
			opts.selected_index = self.cursor
			self.cursor = 1
		end
	...
	if use_virtual_directory then
		...
		if opts.directory_label then
			...
			if opts.directory_label:find("Favourites/") ~= 1 then
...
local function write_to_file()
	...
	if not file then
		return msg.error(file, "could not open favourites file")
	end
...
local function move_favourite(path, direction)
	...
	if not index or not favourites[index + direction] then
		return
	end
...
local function toggle_favourite(cmd, state, co)
	...
	if state.directory:find("[fF]avourites/$") then
...
local function move_key(cmd, state, co)
	if not state.directory:find("[fF]avourites/") then
		return false
	end
	local path = favs.get_full_path(state.list[state.selected], state.directory)
...
mp.register_script_message("favourites/move_up", function(path)
	move_favourite(path, -1)
end)
mp.register_script_message("favourites/move_down", function(path)
	move_favourite(path, 1)
end)
