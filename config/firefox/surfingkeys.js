...
mapkey("ymo", "以org格式复制链接", () => {
	...
	if (domain === "github.com") {
		const urlEnd = url.replace("https://github.,com", "");
...
mapkey("ymf", "复制网站ico", () => {
	...
	if ("" === c) {
		const url = `http://${window.location.host}/favicon.ico`;
		if (UrlExists(url)) {
			c = url;
		}
	}
...
// 暂时不显示图片，便于阅读
mapkey(",tm", "Toggle images blocker", () => {
...
mapkey("op", "display password", () => {
	document.querySelectorAll(
		"input[type=password]".forEach((dom) => {
			dom.setAttribute("type", "text");
		}),
	);
});

vmapkey("e", "Org Capture", () => {
	location.href =
		"org-protocol://capture?" +
		new URLSearchParams({
			template: "ip",
...
api.Front.registerInlineQuery({
	url: (q) => `http://dict.youdao.com/w/eng/${q}/#keyfrom=dict2.index`,
	parseResult: (res) => {
...
`;
//@https://github.com/brookhong/Surfingkeys/pull/1898
// 为什么不支持 prefers-color-scheme
// dark_theme 来自: https://github.com/rose-pine/surfingkeys/blob/main/dist/rose-pine.js
// light_theme 来自: https://github.com/rose-pine/surfingkeys/blob/main/dist/rose-pine-dawn.js
settings.theme = `
