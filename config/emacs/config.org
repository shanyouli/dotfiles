...
(defun init-default-frame (&optional frame)
  (interactive)
  (let ((x-width (or (alist-get 'width default-frame-alist)
                     (truncate (/ (- (* (x-display-pixel-width) 0.5) 24)
                                  (frame-char-width)))))
        (y-height (or (alist-get 'height default-frame-alist)
                      (truncate (/ (* (x-display-pixel-height) 0.5)
                                   (frame-char-height))))))
...
(use-package! rime
  ...
  ;; fix emacs-rime with evil-escape
...
(use-package! insert-translated-name
  ...
  (defun int/active-a (&rest _)
    ...
    (when (boundp 'int--sis-default-input-thmod)
     ...
     (unless (bound-and-true-p using-emacs-rime-p)
